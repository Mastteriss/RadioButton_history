    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        buttonBTNTV.setOnClickListener {
            ...
            if(selectedId != -1){
                ...
                when(selectedId){
                    R.id.oneRB ->{
                        if(selectedRadio.text == "a) Рюриковичи"){
                            counterValue += 0
                        }
                    }
                    R.id.twoRB -> {
                        if (selectedRadio.text == "b) Годуновы") {
                            counterValue += 0
                        }
                    }
                    R.id.threeRB -> {
                        if (selectedRadio.text == "c) Романовы") {
                            counterValue += 100
                        }
                    }
                    R.id.fourRB -> {
                        if (selectedRadio.text == "d) Черкасские") {
                            counterValue += 0
                        }
                    }
                }

                val nextIntent = Intent(this, Question_6::class.java)
